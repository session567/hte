import { describe, expect, it } from 'vitest'

import { createElement } from '@/entrypoints/content/common/test/utils'
import { calcWeekNumber, parseDate } from '@/entrypoints/content/modules/week-number/utils'

describe(parseDate, () => {
  it.each([
    { html: '15.03.2024 14:30', expected: new Date(Date.UTC(2024, 2, 15, 14, 30)) },
    { html: '31.12.2023', expected: new Date(Date.UTC(2023, 11, 31)) },
  ])('parses date from text: $html', ({ html, expected }) => {
    const element = createElement(html)

    expect(parseDate(element)).toStrictEqual(expected)
  })

  it.each([
    { desc: 'empty text', html: '' },
    { desc: 'invalid format', html: 'invalid-format' },
  ])('returns null for $desc', ({ html }) => {
    const element = createElement(html)

    expect(parseDate(element)).toBeNull()
  })
})

describe(calcWeekNumber, () => {
  it.each([
    { date: new Date(Date.UTC(2025, 11, 7, 21, 55)), expectedWeek: 16 },
    { date: new Date(Date.UTC(2025, 2, 4, 19, 45)), expectedWeek: 9 },
    { date: new Date(Date.UTC(2025, 0, 4, 13, 20)), expectedWeek: 16 },
    { date: new Date(Date.UTC(2024, 8, 17, 19, 30)), expectedWeek: 1 },
    { date: new Date(Date.UTC(2024, 2, 18, 1)), expectedWeek: 7 },
  ])('calculates week number for $date as W$expectedWeek', ({ date, expectedWeek }) => {
    expect(calcWeekNumber(date)).toBe(expectedWeek)
  })
})
