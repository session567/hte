...
const hatstats: Module = {
  metadata,
  pages: [pages.matchDetail.senior, pages.matchOrder],
  run: () => {
    if (isCurrentPage(pages.matchDetail.senior)) return runMatchDetail()
    if (isCurrentPage(pages.matchOrder)) return runMatchOrder()
  },
}
